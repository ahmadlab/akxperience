...
/**
 * Techinal Complain Management Class
 * @author    : Latada
 * @email    : mac_ [at] gxrg [dot] org
 * @type    : Controller
 * @desc    : Techinal Complain Management
 *********************************************/
class Tech_complain extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ctrl = 'tech_complain';
        $this->template = getAdminFolder() . '/modules/tech_complain';
        $this->path_uri = getAdminFolder() . '/tech_complain';
        $this->path = site_url(getAdminFolder() . '/tech_complain');
        $this->title = get_admin_menu_title('tech_complain');
        $this->id_menu_admin = get_admin_menu_id('tech_complain');
    ...
    public function main()
    {
        ...
        $total_records = $this->Technical_model->TotalTechComplain(myUrlDecode($s_about), myUrlDecode($s_status),
        ...
        $query = $this->Technical_model->GetAllTechComplain(myUrlDecode($s_about), myUrlDecode($s_status),
        ...
        if (is_array($query)) {
            ...
            foreach ($query as $buff) {
                ...
                $edit_href = site_url($path_uri . '/reply/' . $buff['id_tech_complain']);
                $list_data[] = array(
                    'no' => $no,
                    'id' => $id,
                    'maker' => $maker,
    ...
    public function change_publish($id = 0)
    {
        if ($id) {
            if (ctype_digit($id)) {
                $return = $this->Technical_model->ChangePublishComplain($id);
                echo $return;
            }
        }
    }
    ...
    public function reply($Id)
    {
        ...
        if (($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($Id)) {
            ...
            $message = 'You received new message on menu Customer Relation';
//            echo var_dump($client); exit;
            ...
            // insert data
            $id = $this->Technical_model->InsertTechComplain($data_post);
    ...
    private function getForm($id = 0)
    {
        ...
        $history = $this->Technical_model->getAllComplainHistory($id);
        if ($history) {
            $heads = array(
                array(
