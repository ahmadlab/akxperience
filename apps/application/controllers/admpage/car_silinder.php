 ...
 *********************************************/
class Car_silinder extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ctrl = 'car_silinder';
        $this->template = getAdminFolder() . '/modules/car_silinder';
        $this->path_uri = getAdminFolder() . '/car_silinder';
        $this->path = site_url(getAdminFolder() . '/car_silinder');
        $this->title = get_admin_menu_title('car_silinder');
        $this->id_menu_admin = get_admin_menu_id('car_silinder');
        $this->is_superadmin = adm_is_superadmin(adm_sess_userid());
        $this->load->model(getAdminFolder() . '/ref_car_silinder', 'Ref_car_silinder');
    }
    ...
    public function main()
    {
        ...
        $total_records = $this->Ref_car_silinder->TotalCarsCc(myUrlDecode($s_cc));

        if ($s_cc) {
            $dis_urut = "none";
            $path = str_replace("/t/", "/t" . $s_cc . "/", $path);
            ($search_query == '' ? $search_query .= $s_cc : $search_query .= ' + ' . $s_cc);
        }
        ...
        $query = $this->Ref_car_silinder->GetAllCarsCc(myUrlDecode($s_cc), $lmt, $per_page);
    ...
    public function change_publish($id = 0)
    {
        if ($id) {
            if (ctype_digit($id)) {
                $return = $this->Ref_car_silinder->ChangePublishCc($id);
                echo $return;
            }
        }
    }
    ...
    {
        auth_admin();
        $s_cc = myUrlEncode(trim($this->input->post('s_cc')));
        redirect($this->path . '/main/t' . $s_cc);
    }
        ...
        if ($this->input->post('id') != "") {
            if (($this->input->post('id') > 0)) {
                if (auth_access_validation(adm_sess_usergroupid(), $this->ctrl)) {
                    ...
                    $this->Ref_car_silinder->DeleteCarsCc($id);
                    $id = implode(', ', $id);
                    #insert to log
    ...
    public function add()
    {
        ...
        if (($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            // insert data
            $id = $this->Ref_car_silinder->InsertCarsCc($data_post);

            #insert to log
            $log_id_user = adm_sess_userid();
            $log_id_group = adm_sess_usergroupid();
            $log_action = 'Add ' . $this->title . ' ID : ' . $id;
            $log_desc = 'Add ' . $this->title . ' ID : ' . $id . '; Car Cc :' . $post['cc'];
            $data_logs = array(
                'id_user' => $log_id_user,
                'id_group' => $log_id_group,
                'action' => $log_action,
    ...
    public function edit($get_id = 0)
    {
        ...
        if (($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id)) {
            ...
            $this->Ref_car_silinder->UpdateCarsCc($data_post, $id);
    ...
    private function getForm($id = 0)
    {
        ...
        if ($id) {
            $query = $this->Ref_car_silinder->GetCarsCcById($id);
            if ($query->num_rows() > 0) {
        ...
        if (isset($this->error['cc'])) {
            $error_msg['cc'] = alert_box($this->error['cc'], 'error');
        } else {
            $error_msg['cc'] = '';
        }
    ...
    private function validateForm($id = 0)
    {
        ...
        if ($post['cc'] == '') {
            $this->error['cc'] = 'Please insert Car CC.<br/>';
        } else {
            if (!$this->Ref_car_silinder->CheckExistsCarsCc($post['cc'], $id)) {
                $this->error['cc'] = 'Car CC already exists, please input different Car CC.<br/>';
            } else {
                if (utf8_strlen($post['cc']) < 3) {
                    $this->error['cc'] = 'Car CC length must be at least 5 character(s).<br/>';
                }
            }
        }
