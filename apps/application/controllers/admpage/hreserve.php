...
/**
 * Service Home Reserve Management Class
 * @author    : Latada
 * @email    : mac_ [at] gxrg [dot] org
 * @type    : Controller
 * @desc    : Service Home Reserve Management
 *********************************************/
class Hreserve extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ctrl = 'hreserve';
        $this->template = getAdminFolder() . '/modules/hreserve';
        $this->path_uri = getAdminFolder() . '/hreserve';
        $this->path = site_url(getAdminFolder() . '/hreserve');
        $this->title = get_admin_menu_title('hreserve');
        $this->id_menu_admin = get_admin_menu_id('hreserve');
    ...
    public function main()
    {
        ...
        $total_records = $this->Reserve_model->TotalServiceHomeReserve(myUrlDecode($s_name), myUrlDecode($s_status),
        ...
        $query = $this->Reserve_model->GetAllServiceHomeReserve(myUrlDecode($s_name), myUrlDecode($s_status),
    ...
    private function getForm($id = 0)
    {
        ...
        if ($id) {
            $query = $this->Reserve_model->GetServiceHomeReserveById($id);
            if ($query->num_rows() > 0) {
    ...
    public function exportoxl()
    {
        ...
        export_to('Service_Booking_Home_Date_' . date('Ymd') . '.xls');
