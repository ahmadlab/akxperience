 ...
 *********************************************/
class Sreserve extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ctrl = 'sreserve';
        $this->template = getAdminFolder() . '/modules/sreserve';
        $this->path_uri = getAdminFolder() . '/sreserve';
        $this->path = site_url(getAdminFolder() . '/sreserve');
        $this->title = get_admin_menu_title('sreserve');
        $this->id_menu_admin = get_admin_menu_id('sreserve');
    ...
    public function main()
    {
        ...
        $s_stype = $this->uri->segment(8);
        $pg = $this->uri->segment(9);
        $per_page = 25;
        $uri_segment = 9;
        $no = 0;
        $path = $this->path . '/main/a/b/c/d/e/';
        ...
        $l_stype = $this->list_stype($s_stype);
        $l_status = $this->list_status($s_status);
        $total_records = $this->Reserve_model->TotalServiceReserve(myUrlDecode($s_name), myUrlDecode($s_status),
            myUrlDecode($s_sdate), myUrlDecode($s_edate), $this->location_id, myUrlDecode($s_stype));
        ...
        if ($s_stype) {
            $path = str_replace("/e/", "/e" . $s_stype . "/", $path);
            ($search_query == '' ? $search_query .= $s_stype : $search_query .= ' + ' . $s_stype);
        }
        ...
        $query = $this->Reserve_model->GetAllServiceReserve(myUrlDecode($s_name), myUrlDecode($s_status),
            myUrlDecode($s_sdate), myUrlDecode($s_edate), $lmt, $per_page, $this->location_id, myUrlDecode($s_stype));
        ...
        foreach ($query->result_array() as $buffs) {
            ...
            $service_type = $buffs['service_type'];
    ...
    public function search()
    {
        ...
        $s_stype = myUrlEncode(trim($this->input->post('s_stype')));
        redirect($this->path . '/main/a' . $s_name . '/b' . $s_status . '/c' . $s_sdate . '/d' . $s_edate . '/e' . $s_stype);
    }
    ...
    public function get_worktime()
    {
        ...
        $data = array('status' => '0');
        if ($locate) {
            $worktime = $this->db->get_where('ref_time_work', array('ref_location' => $locate, 'type' => 'service'));
    ...
    public function store_worktime()
    {
        ...
        if (isset($post['wlocation']) && isset($post['starttime']) && isset($post['endtime']) && isset($post['interval'])) {
            $update = db_get_one('ref_time_work', 'ref_location',
                "ref_location = '" . $post['wlocation'] . "' AND type = 'service'");
            $dupdate = array(
    ...
    public function list_stype($selected = false)
    {
        $opt = "<option value=''>--- Service Type ---</option>";
        foreach (array('home', 'workshop') as $l) {
    ...
    public function list_status($selected = false)
    {
        ...
        } elseif ($group == 1 || $group == 2 || $group == 9 || $group == 3) {
            $status_array = array(
                'booked',
                'confirmed',
                'canceled',
    ...
    private function getForm($id = 0)
    {
        ...
        $eta = explode(' ', $info['eta']);
        $info['eta_1'] = $eta['0'];

        if ($info['booked_by'] != '') {
            $status_info['booked_by'] = '<tr><td colspan="2">Booked by ' . $info['booked_by'] . ' at ' . $info['booked_at'] . '</td></tr>';
        } else {
            $status_info['booked_by'] = '<tr><td colspan="2">Booked by client using mobile app at ' . $info['create_date'] . '</td></tr>';
        }
        if ($info['confirmed_by'] != '') {
            $status_info['confirmed_by'] = '<tr><td colspan="2">Confirmed by ' . $info['confirmed_by'] . ' at ' . $info['confirmed_at'] . '</td></tr>';
        } else {
            $status_info['confirmed_by'] = '';
        }
        if ($info['progress_by'] != '') {
            $status_info['progress_by'] = '<tr><td colspan="2">Progress by ' . $info['progress_by'] . ' at ' . $info['progress_at'] . '</td></tr>';
        } else {
            $status_info['progress_by'] = '';
        }
        if ($info['done_by'] != '') {
            $status_info['done_by'] = '<tr><td colspan="2">Done by ' . $info['done_by'] . ' at ' . $info['done_at'] . '</td></tr>';
        } else {
            $status_info['done_by'] = '';
        }
        if ($info['finished_by'] != '') {
            $status_info['finished_by'] = '<tr><td colspan="2">Finished by ' . $info['finished_by'] . ' at ' . $info['finished_at'] . '</td></tr>';
        } else {
            $status_info['finished_by'] = '';
        }
        if ($info['canceled_by'] != '') {
            $status_info['canceled_by'] = '<tr><td colspan="2">Canceled by ' . $info['canceled_by'] . ' at ' . $info['canceled_at'] . '</td></tr>';
        } else {
            $status_info['canceled_by'] = '';
        }

        $post = array($post);
        $info = array($info);
        $status_info = array($status_info);
        $error_msg = array($error_msg);

        $form = ($id) ? $file_app : $file_app . '_add';

        $add_url = site_url(getAdminFolder() . '/list_user_member/add');
        ...
        $data = array(
            ...
            'error_msg' => $error_msg,
            'breadcrumbs' => $breadcrumbs,
            'code' => addslashes(json_encode($df)),
            'file_app' => $file_app,
            'path_app' => $path_app,
