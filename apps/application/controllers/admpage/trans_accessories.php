...
/**
 * Transaction Accessories Management Class
 * @author    : Latada
 * @email    : mac_ [at] gxrg [dot] org
 * @type    : Controller
 * @desc    : Transaction Accessories Management
 *********************************************/
class Trans_accessories extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ctrl = 'trans_accessories';
        $this->template = getAdminFolder() . '/modules/trans_accessories';
        $this->path_uri = getAdminFolder() . '/trans_accessories';
        $this->path = base_url() . getAdminFolder() . '/trans_accessories';
        $this->title = get_admin_menu_title('trans_accessories');
        $this->id_menu_admin = get_admin_menu_id('trans_accessories');
        $this->is_superadmin = adm_is_superadmin(adm_sess_userid());
        $this->max_thumb_height = 105;
        $this->max_thumb_width = 148;
        $this->path_thumb = './uploads/trans_accessories/';
    ...
    public function main()
    {
        ...
        $total_records = $this->Trans_model->TotalTransAccessories(myUrlDecode($s_usr));
        ...
        $query = $this->Trans_model->GetAllTransAccessories(myUrlDecode($s_usr), $lmt, $per_page);

        foreach ($query->result_array() as $buffs) {
            $no++;
            $id = $buffs['id_car_accessories_transaction'];
            $usr = db_get_one('user', 'username', "id_user = '" . $buffs['id_user'] . "'");
            $accessories = db_get_one('car_accessories', 'accessories',
                "id_car_accessories = '" . $buffs['id_car_accessories'] . "'");
    ...
    public function add()
    {
        ...
        if (($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $data_post = array(
                ...
                'id_car_accessories' => $post['ref_accessories'],
            );

            $id = $this->Trans_model->InsertTransAccessories($data_post);
            ...
            $log_desc = 'Add ' . $this->title . ' ID : ' . $id . '; Ref User:' . $post['ref_user'] . '; Ref Accessories:' . $post['ref_accessories'];
            $data_logs = array(
                'id_user' => $log_id_user,
    ...
    public function edit($get_id = 0)
    {
        ...
        if (($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id)) {
            ...
            $id = $this->Trans_model->UpdateTransAccessories($data_post, $id);
    ...
    private function getForm($id = 0)
    {
        ...
        if ($id) {
            $query = $this->Trans_model->GetTransAccessoriesById($id);
            if ($query->num_rows() > 0) {
        ...
        if ((int)$id > 0) {
            ...
            $accessorieslist = selectlist('car_accessories', 'id_car_accessories', 'accessories', "ref_publish = '1'",
                $info['id_car_accessories'], '--- Chooice Accessories ---');
        } else {
            $usrlist = selectlist('user', 'id_user', 'username', null, null, '--- Chooice User ---');
            $accessorieslist = selectlist('car_accessories', 'id_car_accessories', 'accessories', "ref_publish = '1'",
                null, '--- Chooice Accessories ---');
        }
    ...
    private function validateForm($id = 0)
    {
        ...
        if ($post['ref_accessories'] == '' || $post['ref_accessories'] == 0) {
            $this->error['ref_accessories'] = 'Please select Accessories.<br/>';
        }
